...
namespace video {
    ...
    void draw_rect(unsigned int x, unsigned int y, unsigned int width, unsigned int height, uint8_t r, uint8_t g, uint8_t b) {
        ...
        for(unsigned i = 0; i < height; i++) {
            for(unsigned j = 0; j < width; j++) {
                ((uint32_t*)video_memory)[((i + y) * screen_width) + j + x] = color;
            }
        }
    }

    void draw_char_safe(char c, unsigned int x, unsigned int y, uint8_t r, uint8_t g, uint8_t b) {
        uint32_t color = r << 16 | g << 8 | b;
        for (unsigned i = 0; i < 8; i++) {
            int row = default_font[(int)c][i];
            for (unsigned j = 0; j < 8; j++) {
                if ((row & (1 << j)) >> j) {
                    ((uint32_t*)video_memory)[((y + i) * screen_width) + x + j] = color;
                }
            }
        }
    }
    ...
    void draw_char(char c, unsigned int x, unsigned int y, uint8_t r, uint8_t g, uint8_t b) {
        uint32_t color = r << 16 | g << 8 | b;
        char* font_ptr = (char *)font->glyphs + (c * font->header.char_size);
        for (unsigned i = 0; i < 16; i++) {
            for (unsigned j = 0; j < 8; j++) {
                if ((*font_ptr & (0b10000000 >> j)) > 0) {
                    ((uint32_t*)video_memory)[((y + i) * screen_width) + x + j] = color;
                }
            }
    ...
    void draw_string(const char* str, unsigned int x, unsigned int y, uint8_t r, uint8_t g, uint8_t b, bool fallback) {
        int x_off = 0;
        while(*str != 0) {
            if(fallback) {
                draw_char_safe(*str, x + x_off, y, r, g, b);
            } else {
                draw_char(*str, x + x_off, y, r, g, b);
            }
